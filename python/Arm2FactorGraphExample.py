...
from gpmp_utils.plotPlanarArm import plotPlanarArm
from gpmp_utils.plotRobotModel2D import plotRobotModel2D
# small dataset
...
plotEvidenceMap2D(figure2, axis2, dataset.map, dataset.origin_x, dataset.origin_y, cell_size);
...
for i in range(0, total_time_step+1):
    ...
    if i > 0:
        ...
        #% cost factor
        graph.push_back(ObstaclePlanarSDFFactorArm(key_pos, arm, 
                                sdf, cost_sigma, epsilon_dist))

        graph_obs.push_back(ObstaclePlanarSDFFactorArm(
...
axis3 = figure3.gca()
# plot world
plotEvidenceMap2D(figure3, axis3, dataset.map, dataset.origin_x, dataset.origin_y, cell_size)
