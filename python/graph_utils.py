...
def get_initializations(nr_chains, problem):
  ...
  for i in range(0, problem.total_time_step+1):
    ...
    pose = \
      problem.start_conf * \
        float(problem.total_time_step-i)/float(problem.total_time_step) +\
    ...
    # start/end priors
    if i==0:
      graph.push_back(PriorFactorVector(key_pos, problem.start_conf, 
                                                  problem.pose_fix_model))
      graph.push_back(PriorFactorVector(key_vel, problem.start_vel, 
                                                  problem.vel_fix_model))
    elif i==problem.total_time_step:
      graph.push_back(PriorFactorVector(key_pos, problem.end_conf, 
                                                  problem.pose_fix_model))
      graph.push_back(PriorFactorVector(key_vel, problem.end_vel, 
                                                  problem.vel_fix_model))
    ...
    if i > 0:
      ...
      temp = problem.gp_factor_function(key_pos1, key_vel1,
  ...
  for i in range(nr_chains):
    initializations.append(samples[i,:].reshape((problem.total_time_step+1, 
                                                problem.start_conf.shape[0])))
  ...
  for i in range(nr_chains): # go through each chain
    for j in range(1,total_time_step-1): # go through each time point
      for k in range(nr_chains): # choose a branch to connect to
        ...
        rand_num = random()
        if random < problem.dropout_prob:
          first_idx = map_[(i,j)]
...
def get_gtsam_graph(node_list, problem):
  ...
  for i in range(len(node_list)):
    ...
    #% start/end priors
    if i==0:
      graph.push_back(PriorFactorVector(key_pos, node_list[i].pose, 
                                            problem.pose_fix_model))
      graph.push_back(PriorFactorVector(key_vel, problem.start_vel, 
                                                problem.vel_fix_model))
    elif i==1:
    ...
    if i > 0:
      #% cost factor
      graph.push_back(problem.obstacle_factor_function(
            key_pos, problem.gpmp_robot, problem.sdf, 
    ...
    for neigh_id in node_list[i].neighbours:
      ...
      graph.push_back(problem.gp_factor_function(key_pos1, key_vel1,
            key_pos2, key_vel2, problem.delta_t, problem.Qc_model))
      ...
      if problem.use_GP_inter and problem.check_inter > 0:
        for j in range(1, problem.check_inter+1):
          ...
          graph.push_back(problem.obstalce_gp_factor_function(
                    key_pos1, key_vel1, key_pos2, key_vel2,
                    problem.gpmp_robot, problem.sdf, problem.cost_sigma, 
