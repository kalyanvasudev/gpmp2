...
def add_obstacle(position, size, map, corner):
    half_size_row = math.floor((size(0)-1)/2)
    half_size_col = math.floor((size(1)-1)/2)
    half_size_z = math.floor((size(2)-1)/2)
    ...
    #% corner
    temp = np.asarray([position[0]-half_size_row-1 , position[0]+half_size_row-1,
                        position[1]-half_size_col-1 , position[1]+half_size_col-1,
                        position[2]-half_size_z -1  , position[2]+half_size_z-1])
    if corner is None:
        corner = temp
    else:
...
def generate3Ddataset(dataset_str):
    ...
    if dataset_str is 'SmallDemo':
        ...
        dataset.map, dataset.corner_idx = 
                add_obstacle([150 150 150], [20, 20, 20], dataset.map, dataset.corner_idx)
